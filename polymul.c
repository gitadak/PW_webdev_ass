...
poly* mul(poly *f,poly *p)
{
    poly *i,*s=NULL,*newpoly;
    for(i=f;i!=NULL;i=i->next)
    {
        newpoly=create(p->coef*i->coef,p->exp+i->exp);
        s=insert(s,newpoly);
    }
    return s;    
}

poly* poly_mul(poly *f1,poly *f2)
{
    poly *temp_poly=NULL,*final_poly=NULL,*i;
    for(i=f1;i!=NULL;i=i->next)
    {
        temp_poly=mul(f2,i);
        final_poly=poly_add(final_poly,temp_poly);
    }
    return final_poly;
}
